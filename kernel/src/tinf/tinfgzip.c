...
int TINFCC ktinf_gzip_uncompress(const void *source, unsigned int sourceLen,
        unsigned int* dest, unsigned int* destLen)
{
    const unsigned char *src = (const unsigned char *) source;
    const unsigned char *start;
    unsigned int dlen, crc32;
    ...
    //printf("Allocating %d frames for %d bytes\n", FRAME_SIZE - (dlen) % FRAME_SIZE + dlen, dlen);
    *dest = (unsigned int) kalloc_frames(FRAME_SIZE - (dlen) % FRAME_SIZE + dlen);
    ...
    res = tinf_uncompress(dest, destLen, start,
            (src + sourceLen) - start - 8);
